...
class TransactionSeeder extends Seeder
{
    ...
    {
        $laundry = Laundry::where('laundry_kode', 'LDRY001')->first();
        if (! $laundry) {
            $this->command->warn('Laundry LDRY001 not found, run LaundryDemoSeeder first.');
            return;
        }

        $statuses = OrderStatus::where('order_status_id_laundry', $laundry->laundry_id)->orderBy('order_status_urutan')->get();
        if ($statuses->isEmpty()) {
            $this->command->warn('No order_status found for laundry.');
            return;
        }
        ...
        for ($i = 0; $i < 50; $i++) {
            ...
            if (! $itemProducts instanceof \Illuminate\Support\Collection) {
