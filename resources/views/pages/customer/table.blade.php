            ...
            <x-slot:body>
                @foreach ($data as $table)
                    <tr>
                        ...
                        <td>{{ $table->customer_telepon }}</td>
                        <td>{{ $table->customer_email ?? '-' }}</td>
                    </tr>
                @endforeach
            </x-slot:body>
            ...
            <x-slot:mobile>
                ...
                <div class="p-3 space-y-3" id="mBody">
                    ...
                    <div class="border border-outline-variant rounded-xl p-4 bg-surface-container-lowest shadow-sm" data-id="{{ $table->field_primary }}">
                        <p class="text-sm font-bold text-on-surface truncate mb-3">{{ $table->customer_nama }}</p>
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <div class="text-left">
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Telepon</p>
                                <p class="text-xs font-medium text-primary">{{ $table->customer_telepon }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Email</p>
                                <p class="text-xs font-medium text-on-surface truncate">{{ $table->customer_email ?? '-' }}</p>
                            </div>
                        </div>
